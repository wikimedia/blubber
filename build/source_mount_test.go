...
func TestSourceMount(t *testing.T) {
	t.Run("without destination and source path", func(t *testing.T) {
		...
		// Note that the mount comes before / in this case as mounts are sorted by destination
		// path.
		mnt := eops[1].Exec.Mounts[0]
